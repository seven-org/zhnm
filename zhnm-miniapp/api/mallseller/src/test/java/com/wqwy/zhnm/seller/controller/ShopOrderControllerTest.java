...
public class ShopOrderControllerTest {
    ...
    public void testGetSellerShopOrders() throws Exception {
        mockMvc.perform(get("/v1/shopOrders/seller/11?pageNum=1&pageSize=20").header(DefaultConstants.TOKEN, TestConsts.TokenValue))
        .andDo(print())
    ...
    public void testGetSellerAllOrders() throws Exception {
        mockMvc.perform(get("/v1/orders/seller/15?searchString=13800138000&pageNum=1&pageSize=20").header(DefaultConstants.TOKEN, TestConsts.TokenValue))
        .andDo(print())
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        .andReturn().getResponse().getContentAsString();
