...
Page({ 
  ...
  */
  ...
  onLoad: function (options) {
    ...
    // wx.request({
    //   url: that.data.httpserver + 'goods/info',
    ...
    util.request(app.data.httpserver + 'goods/info', data = data).then(function (request) {   
        ...
        var goods_detail = request.goods.goods_detail.replace(/src="/g, 'src="').replace('<img', '<img mode="widthFix" style="width:100%"');
        ...
        that.setData({
          ...
          //   }]
          // }], 
          goods_price: goods.goods_price.toFixed(2),
  ...
  */
    ...
    if (count == attr_key_value_list.length) {//如果选择数等于数组属性数，则选择完毕
      ...
      for (var i = 0; i < attr_list.length; i++) {
        if (reday_attr_arry.toString() == attr_list[i].goods_attr.toString()) {
          ...
          this.setData({
            goods_price: goods_detail.attribute_detail_price.toFixed(2),
  ...
  add_cart() {
    ...
    // wx.request({
    //   url: that.data.httpserver + 'cart/add',
    //   type: 'post',
    //   data: { goods_id: goods_id, goods_count: goods_count, attribute_detail_id: attribute_detail_id },
    //   header: wx.getStorageSync('header'),
    //   success: function (request) {
    var data = { goods_id: goods_id, goods_count: goods_count, attribute_detail_id: attribute_detail_id }
  ...
  */
  ...
  collection() {
    ...
    var goods_id = that.data.goods.goods_id;
    // wx.request({
    //   url: that.data.httpserver + 'collection/add',
    //   type: 'post',
    //   data: { goods_id: goods_id },
    //   header: wx.getStorageSync('header'),
    //   success: function (request) {
  ...
  collection_ed() {
    ...
    //   header: wx.getStorageSync('header'),
    //   success: function (request) {
    util.request(app.data.httpserver + 'collection/delete', data = { goods_id: goods_id }).then(function (request) { 
          that.setData({
            collection: null,
          });
          wx.showToast({
